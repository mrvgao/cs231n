...
def relu_backward(dout, cache):
  ...
  """
    ...
    ## activate(x) = max(x, 0)
    ## partial_activate / partial_x = 1 if x > 0
    ## 0 else
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  """
    ...
    if mode == 'train':
        ...
        cache = (running_mean, running_var, gamma, beta, x, eps)
...
def batchnorm_backward(dout, cache):
  ...
  """
    ...
    running_mean, running_var, gamma, beta, x, eps = cache
    ...
    # dx = dout * gamma/np.sqrt(running_var + esp)

    dx = dout * gamma / np.sqrt(running_var + eps)

    # dr = (x - running_mean) / (np.sqrt(running_var) + eps) \dot dout
    # (D, ) = (N, D) * (N, D)

    dgamma = np.sum(dout, axis=0) * np.sum( ((x - running_mean)/(np.sqrt(running_var) + eps)), axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  """
    ...
    '''
    r, c = -1, -1  # initial row and column's index
    for n in range(N):
        for f in range(F):

            for col in range(H_out):
                c = col * stride

                for row in range(W_out):
                    r = row * stride
                    window = x_pad[n, :, c: c+HH, r: r+WW]
                    conv = np.sum(window * w[f] + b[f])
                    out[n, f, col, row] = conv
    '''
        ...
        for f in range(F):
            for h_i in range(H_out):
                ...
                for w_i in range(W_out):
                    ...
                    out[n, f, h_i, w_i] = np.sum(window * w[f]) + b[f]
